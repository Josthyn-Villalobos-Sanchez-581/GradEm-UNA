...
export default function OfertaDetalle({
  ...
  const tipo = oferta.tipo_oferta?.toLowerCase() ?? "";
  const sector = oferta.categoria ?? "";
  const metodologiaRaw = oferta.modalidad?.nombre?.toLowerCase() ?? "";

  const metodologia =
    metodologiaRaw.charAt(0).toUpperCase() + metodologiaRaw.slice(1);
  ...
  return (
    ...
    <div className="w-full bg-[#F8FAFC] md:min-h-screen">
      <div className="max-w-6xl mx-auto bg-white md:rounded-[2.5rem] shadow-sm md:shadow-2xl border-x md:border border-gray-100 overflow-hidden">
        ...
        {/* ================= HEADER MODERNO ================= */}
        <div className="relative bg-[#034991] py-6 px-6 sm:py-8 sm:px-8 md:py-10 md:px-12 text-white overflow-hidden">
            ...
            <div className="space-y-4 md:space-y-6 text-center md:text-left flex-1 w-full">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-2">
                  ...
                  <span className="px-4 py-2 bg-[#CD1719] rounded-xl text-xs md:text-sm font-black uppercase tracking-wider">
                    {sector || "General"}
                  </span>


                  {/* Ubicación */}
                  <span className="flex items-center gap-2 text-sm md:text-base font-bold bg-white/10 px-4 py-2 rounded-xl backdrop-blur-sm">
                    <MapPin className="w-4 h-4 text-[#CD1719]" />
                    <span className="truncate max-w-[220px]">{ubicacion || "Costa Rica"}</span>
                  </span>
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="flex-1 p-6 md:p-10 space-y-8 md:space-y-12 order-2 lg:order-1">
            ...
            {/* Requisitos */}
{requisitos.length > 0 && (
  <section className="bg-gray-50/80 p-5 md:p-6 rounded-2xl md:rounded-[2.5rem] border border-gray-100 relative">

    {/* Título flotante más compacto */}
    <div className="absolute -top-3 left-6 bg-white px-4 py-1 rounded-lg shadow-sm border border-gray-100 flex items-center gap-2">
      <CheckCircle2 className="text-[#CD1719] w-4 h-4" />
      <span className="font-black text-[#034991] text-xs">
        LO QUE BUSCAMOS
      </span>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
      {requisitos.map((r, i) => (
        <div
          key={i}
          className="group flex gap-3 bg-white p-3 md:p-4 rounded-xl border border-transparent hover:border-[#034991]/20 hover:shadow-sm transition-all duration-300"
        >
          <div className="w-5 h-5 rounded-full bg-blue-50 flex items-center justify-center shrink-0 group-hover:bg-[#034991] transition-colors">
            <ChevronRight className="w-3 h-3 text-[#034991] group-hover:text-white" />
          </div>
          ...
          <span className="text-sm font-semibold text-gray-700 leading-snug">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                <div className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-100 h-full">
          ...
          <aside className="w-full lg:w-96 bg-[#FBFBFB] border-t lg:border-t-0 lg:border-l border-gray-100 p-6 md:p-8 order-1 lg:order-2">
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-lg md:shadow-xl shadow-gray-200/50 border border-gray-100 space-y-6 md:space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 lg:gap-0 lg:space-y-6">
                  {[
                    ...
                    {
                      label: "Publicado el",
                      value: formatearFecha(oferta.fecha_publicacion),
                      icon: Calendar,
                    },
                  ].map((item, idx) =>
                    item.value && (
                      <div key={idx} className="group">
                        ...
                        <div className="ml-5 md:ml-7">
                          {item.isBadge ? (
                            <span
                              className={`inline-block px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wider ${tipoStyles}`}
                            >
                              {tipo === "practica" ? "Práctica" : "Empleo"}
                            </span>
                          ) : (
                            <p className="text-xs md:text-sm font-black text-[#034991] line-clamp-2">
                              {item.value}
                            </p>
                          )}
                        </div>
              ...
              {mostrarBotonPostular && (
                <div className="sticky bottom-4 z-20 lg:static">
                  <button
                    ...
                    className={`group w-full py-4 md:py-5 rounded-2xl md:rounded-[2rem] flex justify-center items-center gap-3 font-black transition-all duration-300 shadow-xl lg:shadow-2xl scale-100 active:scale-95 text-sm md:text-base

...
export default function EditarEgresado({
  ...
  const validarCampo = (name: string, value: string | number | null) => {
    ...
    if (name === "fecha_nacimiento") {
      if (!value) error = "Campo obligatorio.";
        ...
        if (year > currentYear - 16) error = "Debe tener mínimo 16 años.";
        else if (year < currentYear - 100) error = "Edad no válida.";
      }
    }
    ...
    if (name === "anio_graduacion") {
      ...
      if (!value) error = "Indique el año.";
      else if (y < 2007 || y > currentYear) error = `Debe estar entre 2007 y ${currentYear}.`;
    }
  ...
  return (
    <>
      <Head title="Editar Perfil de Egresado" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      onBlur={(e) => validarCampo(e.target.name, e.target.value)}
                      className={`mt-1 block w-full p-2 border rounded-md shadow-sm sm:text-sm ${errores.nivel_academico ? "border-red-500" : "border-gray-300"
                        }`}
                    >
                    ...
                    {errores.nivel_academico && <span className="text-red-500 text-xs mt-1">{errores.nivel_academico}</span>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full p-2 border rounded-md shadow-sm sm:text-sm ${errores.anio_graduacion ? "border-red-500" : "border-gray-300"
                        }`}
                    />
                    {errores.anio_graduacion && <span className="text-red-500 text-xs mt-1">{errores.anio_graduacion}</span>}
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      ...
                      {carreras.filter(c => c.id_universidad === selectedUniversidad).map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                    </select>
...
// Layout
EditarEgresado.layout = (page: any) => <PpLayout userPermisos={page.props.userPermisos}>{page}</PpLayout>;
